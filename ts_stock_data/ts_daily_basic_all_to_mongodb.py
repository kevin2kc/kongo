...
def prepStockDate(db, stock_pool, ed_date):
    try:
        results = db.daily_basic.find({'ts_code': stock_pool}).sort("trade_date", pymongo.DESCENDING).limit(1)
    except Exception as exp:
        print(stock_pool + " : ")
        print(exp)

    if db.daily_basic.count_documents({'ts_code': stock_pool}) == 0:
...
def saveStockToMongo(db, data, stock_pool):
    try:
        ...
        db.daily_basic.insert_many(result)
    except Exception as exp:
        print(stock_pool + " : ")
        print(exp)
