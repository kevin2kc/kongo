...
def prepStockDate(db, stock_pool, ed_date):
    ...
    try:
       results = db[stock_pool].find({'ts_code': stock_pool}).sort("trade_date", pymongo.DESCENDING).limit(1)
    except Exception as exp:
        print(stock_pool + " : ")
        print(exp)

    if db[stock_pool].count_documents({'ts_code': stock_pool}) == 0:
...
def saveStockToMongo(db, data, stock_pool):
    try:
        ...
        db[stock_pool].insert_many(result)
        db[stock_pool].create_index([("trade_date", pymongo.DESCENDING)], unique=True)
        db[stock_pool]
